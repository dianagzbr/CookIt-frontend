...
export default function EditRecipeScreen({ route, navigation }) {
  ...
  const [formData, setFormData] = useState({
    recipeName: item.nombre_receta,
    ingredients: item.ingredientes,
    instructions: item.pasos,
    image: item.imagenes.length > 0 ? item.imagenes[0].imagen : null,
    calories: item.calorias.toString(),
    prepTime: item.tiempo_preparacion,
    origin: item.origen_receta,
    difficulty: item.dificultad,
    category: item.categorias[0].id.toString(),
    mealType: item.tipo_comida,
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `https://cookit-j5x3.onrender.com/recetas/${item.id}/`,
        {
          method: "PUT",
          body: form,
          headers: {
      ...
      if (response.ok) {
        setModalData({ content: "Receta actualizada con éxito", title: "Éxito" });
        setIsModalOpen(true);
      } else {
